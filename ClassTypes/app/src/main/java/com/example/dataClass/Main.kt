...
fun main() {
    ...
    val task =
        Task(201, "Implement Feature", "Feature implementation for Project A", dev, 1, "2023-12-31")
    ...
    // переопределил hashCode и equals по полям, а не по ссылкам
    val dev2 = Developer(1, "Alice", "Developer", "Kotlin")
    val set = setOf(dev, dev2)
    val list = listOf(dev, dev2)
    println("set" + set)
    println("list" + list)
    println(set.contains(dev2))
    println(list.contains(dev2))
